...
if ( ! class_exists( 'Awl_Photo_Wall' ) ) {
	...
	class Awl_Photo_Wall {
		...
		protected function _constants() {
			// Plugin Version
			define( 'ALW_PLUGIN_VER', '1.2.8' );

			// Plugin Text Domain
			define( 'ALW_TXTDM', 'animated-live-wall' );

			// Plugin Name
			define( 'ALW_PLUGIN_NAME', 'Animated Live Wall' );
		...
		protected function _hooks() {
			...
			add_action( 'admin_enqueue_scripts', array( $this, 'alw_admin_assets' ) );
		...
		public function alw_menu() {
			add_submenu_page( 'edit.php?post_type=' . ALW_PLUGIN_SLUG, __( 'Filters', 'animated-live-wall' ), __( 'Filters', 'animated-live-wall' ), 'administrator', 'alw-filter-page', array( $this, 'awl_filter_page' ) );
			add_submenu_page( 'edit.php?post_type=' . ALW_PLUGIN_SLUG, __( 'Docs', 'animated-live-wall' ), __( 'Docs', 'animated-live-wall' ), 'administrator', 'alw-doc-page', array( $this, 'alw_doc_page' ) );
		}
		...
		public function awl_filter_page() {
			echo '<div class="wrap"><h1>' . esc_html__( 'Filters', 'animated-live-wall' ) . '</h1><p>' . esc_html__( 'Filter options coming soon.', 'animated-live-wall' ) . '</p></div>';
		}

		/**
		 * Documentation Page
		 */
		public function alw_doc_page() {
			echo '<div class="wrap"><h1>' . esc_html__( 'Documentation', 'animated-live-wall' ) . '</h1><p>' . esc_html__( 'For documentation, please visit', 'animated-live-wall' ) . ' <a href="https://awplife.com/demo/animated-live-wall-free-wordpress-plugin/" target="_blank">awplife.com</a></p></div>';
		}
		...
		public function alw_admin_assets( $hook ) {
			$screen = get_current_screen();
			if ( ! $screen || 'animated_live_wall' !== $screen->post_type ) {
				return;
			}

			// js
			wp_enqueue_media();
			wp_enqueue_script( 'alw-bootstrap-js', plugins_url( 'assets/bootstrap/js/bootstrap.js', __FILE__ ), array( 'jquery' ) );
			wp_enqueue_script( 'alw-uploader-js', plugins_url( 'assets/js/alw-uploader.js', __FILE__ ), array( 'jquery', 'jquery-ui-sortable' ), ALW_PLUGIN_VER, true );
			wp_localize_script( 'alw-uploader-js', 'alw_ajax_obj', array(
				'nonce' => wp_create_nonce( 'alw_ajax_nonce' )
			) );

			// CSS
			wp_enqueue_style( 'alw-bootstrap-css', plugins_url( 'assets/css/bootstrap-min.css', __FILE__ ) );
			wp_enqueue_style( 'alw-metabox-css', plugins_url( 'assets/css/metabox.css', __FILE__ ) );
			wp_enqueue_style( 'alw-admin-css', plugins_url( 'assets/css/alw-admin.css', __FILE__ ) );
			wp_enqueue_style( 'wp-color-picker' );
			wp_enqueue_script( 'alw-metabox-js', plugins_url( 'assets/js/alw-metabox.js', __FILE__ ), array( 'jquery', 'wp-color-picker' ), ALW_PLUGIN_VER, true );

			wp_enqueue_style( 'alw-uploader-css', plugins_url( 'assets/css/alw-uploader.css', __FILE__ ) );
		}
		...
		public function ALW_Shortcode( $post ) { ?>
			...
			<span data-target="#shortcode" class="pw-copy dashicons dashicons-clipboard"></span>
			<?php
		}//end ALW_Shortcode()
		...
		public function _ajax_alw_gallery() {
			if ( ! isset( $_POST['alw_nonce'] ) || ! wp_verify_nonce( $_POST['alw_nonce'], 'alw_ajax_nonce' ) ) {
			...
			if ( isset( $_POST['imageId'] ) ) {
				$image_id = absint( $_POST['imageId'] );
				echo $this->_ig_ajax_callback_function( $image_id );
			}
			wp_die();
		}
		...
		public function _alw_save_settings( $post_id ) {
			if ( isset( $_POST['alw_save_nonce'] ) ) {
				if ( ! wp_verify_nonce( $_POST['alw_save_nonce'], 'alw_save_settings' ) ) {
				...
				if ( ! current_user_can( 'edit_post', $post_id ) ) {
					...
					$alw_grid_rows            = absint( $_POST['alw_grid_rows'] );
					$alw_grid_columns         = absint( $_POST['alw_grid_columns'] );
					...
					$alw_grid_gap             = absint( $_POST['alw_grid_gap'] );
					...
					$alw_images_gap           = absint( $_POST['alw_images_gap'] );
					...
					$alw_custum_css = sanitize_textarea_field( $_POST['alw_custum_css'] );
					...
					$image_ids_val = array_map( 'absint', $image_ids_val );
